...
export default {
  ...
  methods: {
    ...
    initData () {
      var self = this;
      var storeTargetComponent = self.$store.getters.getCurrentProductBTC;
      var storeSelectProduct = self.$store.getters.getSelectProductInfo;
      if (typeof storeSelectProduct === 'string' && self.flag.pFlag) {
        var storeSelectProducts = String(storeSelectProduct).split('_');
    ...
    addProduct () {
      var productItem = this.productItem;
      var selectProduct = this.selectProduct;
      var selectProductId = window.parseInt(selectProduct.id);
      if (productItem.amount >= 1 && productItem.realPrice >= 0 && selectProductId > 0) {
        var productItemInfo = this.selectProduct.id + '_' + this.selectProduct.name + '_' + productItem.realPrice + '_' + productItem.amount + '_' + productItem.sumPrice + '_' + this.selectProduct.times + '_' + this.selectProduct.type;
        this.reset();
